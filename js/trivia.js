document.addEventListener("DOMContentLoaded", () => {
  ...
  formQuestions.addEventListener("submit", (e) => {
    ...
    // --- Vérification que toutes les questions ont une réponse sélectionnée ---
    ...
    if (!allAnswered) {
      alert("You have to select an answer for each question!");
      return; // on sort de la fonction
    }
    btnStartQuiz.classList.remove("d-none");
    btnStartQuiz.textContent = "Start a New Quiz";
        ...
        if (selected.value === correctAnswer) {
          ...
          [...qDiv.querySelectorAll("input")].forEach(inp => {
            if (inp.value === correctAnswer) {
              inp.parentElement.classList.add("text-primary");
            }
          });
        }
      }
